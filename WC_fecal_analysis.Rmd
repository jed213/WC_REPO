...
I will now do a linear mixed model with *shannon* as my response variable, *day*, *sex*, *year*, and *group* as my explanatory variables, and *bird.code* as my random variable to correct for differences between birds. Here is the full equation.
$$
i = bird\ ID
$$
$$
...
**Deterministic part:**
$$
Richness_{det\ ij} = \alpha + \beta_1 (day_{ij}) + \beta_2 (year_{ij}) + \beta_3 (sex_{ij}) + \beta_4 (group_{ij}) + a_i
$$
**Stochastic part for random effect:**
$$
a_i \sim N(0, \sigma^2_a)
$$
Now I will fit the full model, removing day 21 from analysis.
```{r fit model, echo = FALSE}
meta.no21 <- meta.rare %>% 
  filter(day != "21")

model <- lme(shannon ~ day + year + sex + group, 
             random=~1|bird.code, data = meta.no21)
```
I will also compare the AIC with other versions of the model, removing some variables.  
  1. day | bird  
  2. group + day | bird  
  3. group + day + year | bird  
  4. group * day | bird  
  
Below are the AIC values for each model.
```{r fit alternative models, echo = FALSE}
model1 <- lme(shannon ~ day,
             random=~1|bird.code, data = meta.no21)
model2 <- lme(shannon ~ group + day, 
             random=~1|bird.code, data = meta.no21)
model3 <- lme(shannon ~ day + year + group, 
             random=~1|bird.code, data = meta.no21)
model4 <- lme(shannon ~ group * day, 
             random=~1|bird.code, data = meta.no21)
...
Here's where I am stuck: I do not know how to interpret these results since they break group and day into so many smaller variables. For now I will conclude that because of which model fits the data best, there is clearly support for the fact that the effect of day on alpha diversity changes depending what group you are in. But I would like clearer stats/numbers to point to with this. Maybe I can conduct some post-hoc tests?
