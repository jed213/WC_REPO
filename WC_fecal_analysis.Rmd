...
AIC(model6)
```
Out of all the models, the most supported is the full model that includes an interaction term between group and day. I will use that model to look at statistical conclusions about the data going forward, first checking the model fit. 
...
```{r model results, echo = FALSE, warnings = FALSE}
summary(model6)
...
```
Based on the model test using drop1(), adding sex and year to the model doesn't seem to change it's fit very much. But adding the interaction term between day and group makes a huge difference, significantly improving the model. Need to verify why day and group singular terms aren't in the drop1() output/how to interpret that.  
...
Now I will do post-hoc tests between all my combinations of groups to see what is statistically different from each other.  

```{r tukey, echo = FALSE}
# tukey tests
# https://search.r-project.org/CRAN/refmans/biometryassist/html/multiple_comparisons.html
# summary(glht(FeTMB, linfct = mcp(line = ‘Tukey’)))
```
...
```
All terms are significant except for bird.code. Next steps will be post-hoc tests between each of the groups. After will want to find a way to see how the diversity is moving over time.  
