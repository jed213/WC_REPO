...
For differential abundance, I want to compare which taxa are more prevalent between the the treatments.
```{r ANCOMBC2 AR vs PR vs FMT, message = FALSE, warnings = FALSE}
# Pairwise comparisons between all three groups
# This contains all data points except d21
set.seed(14)
ANCOM.all <- ancombc2(data = WC.rare.no21,
                 tax_level = NULL,
                 fix_formula = "group",
                 p_adj_method = "BH",
                 group = "group",
...
```{r plot ANCOMBC2 AR vs PR vs FMT}
# AR vs PR
tax_data <- aggregate_taxa(WC.rare.no21, "Family")
tax_mat <- as(tax_table(tax_data), "matrix")
tax.1 <- as(tax_table(WC.rare.no21), "matrix")
tax.frame <- as.data.frame(tax_table(WC.rare.no21)) %>% 
  mutate(TAXON = row.names(tax.frame))
res <- ANCOM.all$res_pair
df_fig1 <- data.frame(res$lfc_groupPR * res$diff_groupPR, check.names = FALSE) %>% 
...
df_fig = df_fig1 %>% left_join(tax.frame, by = "TAXON")

df_fig
```


When I try to run this analysis including day as a random variable or subsetting to one day, I get the infinite error "Warning: longer object length is not a multiple of shorter object length". I do not know why this error is occuring.
