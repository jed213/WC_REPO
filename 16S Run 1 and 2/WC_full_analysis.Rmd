...
* 55 tracheal samples spread across 28 chicks, 5 years, and 2 timepoints  
...
```{r BA plots by day, warning = FALSE, echo = FALSE}
# day 14
BA %>% 
  filter(Day == "14") %>% 
...
```{r SCFA plots by day, warning = FALSE, echo = FALSE}
# day 14
SCFA %>% 
  filter(Day == "14") %>% 
...
Now cort over time, separated by group and gender.

```{r Cort plotting over time, warning = FALSE, echo = FALSE}
CORT %>% 
  ggplot(aes(x = Day, y = Corticosterone, color = Group, shape = Sex)) +
  ...
  theme_classic(base_size = 15) +
  ylab("Corticosterone (ng/g)")
```

PR seems to start really high (and may be a bit higher overall), through there are not many data points at D14. Female PR cranes specifically had higher cort (need to test statistically), which actually repeats a finding from Barry's paper (Brown et al). I need to test this statistically.  

### Corticosterone stats

```{r Cort stats, warning = FALSE, echo = FALSE}
model <- lm(Corticosterone ~ Day + Group * Sex, data = CORT)
summary(model)
car::Anova(model)
```
...
### Additive plots for talks

These plots remove one group at a time, for use in talks.

```{r F shannon over time additive, include = FALSE}
# these plots were generated for my rotation talk, to add in data sequentially

# shannon but no AR-FMT group
metaF.rare %>%
  filter(day != "21") %>% # remove day 21 samples because only AR had it
  ggplot(aes(x=day, y=shannon, group = group, alpha = group)) + sc +
...
# shannon but no AR-FMT or AR group
metaF.rare %>%
  filter(day != "21") %>% # remove day 21 samples because only AR had it
  ggplot(aes(x=day, y=shannon, group = group, alpha = group)) + sc +
...
```{r F alpha pairs, warning = FALSE, echo = FALSE, message = FALSE}
# Make pairs plot
...
The shannon plot looks more even than observed richness so I will use that one for analysis. I will not look for collinearity since all of my explanatory variables are categorical. I will also remove the DI group from this analysis.
...
Now I will fit the full model. I need to fit using maximum likelihood so that I can compare the later models which have different fixed effects. I am using the *lme4* package.

```{r F alpha fit model, echo = FALSE, warning = FALSE}
metaF.noDI <- metaF.rare %>%
  filter(year != "DI")

model <- lmer(shannon ~ day + group + year + sex + (1 | bird.code), data = metaF.noDI, REML = F)
summary(model)
```
Now I will compare the AIC to other model variations. Namely:  
1. shannon ~ day + group + year + random(bird.code)
2. shannon ~ day + group + random(bird.code)
3. shannon ~ day * group + year + random(bird.code)
4. shannon ~ day * group + random(bird.code)
5. shannon ~ day * group + year + sex + random(bird.code)

```{r F alpha model comparison, echo = FALSE, warning = FALSE}
model1 <- lmer(shannon ~ day + group + year + (1 | bird.code), data = metaF.noDI, REML = F)
model2 <- lmer(shannon ~ day + group + (1 | bird.code), data = metaF.noDI, REML = F)
model3 <- lmer(shannon ~ day * group + year + (1 | bird.code), data = metaF.noDI, REML = F)
model4 <- lmer(shannon ~ day * group + (1 | bird.code), data = metaF.noDI, REML = F)
model5 <- lmer(shannon ~ day * group + year + sex + (1 | bird.code), data = metaF.noDI, REML = F)
```

All the models ran, now I can compare AICs (note: these do give warnings about the fixed-effect model matrix being rank deficient or boundary (singular) fit).

```{r F alpha compare AIC, echo = FALSE, warning = FALSE}
AIC(model)
AIC(model1)
AIC(model2)
AIC(model3)
...
Based on AICs, the best model is the full model with an interaction term between day and group (model 5). This is only slightly better than the model that excludes sex. Now I will check for model validation.

```{r F alpha model validation, echo = FALSE, warning = FALSE}
# Residuals vs fitted values
plot(residuals(model5, type="response") ~ fitted(model5))
abline(0,0,col="gray")
```

These residuals look fine to me. Can proceed with model interpretation.

```{r F alpha model results, echo = FALSE, warning = FALSE, message = FALSE}
summary(model5)
...
Day, group, year, and the interaction between day and group are significant. I will proceed with post-hoc tests to test pairwise differences.
...
```{r F alpha tukey, echo = FALSE}
# gives means for each group/day pair
emm <- emmeans(model5, ~ group * day)
...
These plots will just show the first and last timepoints, hopefully to help illustrate convergence and simplify figures for talks.

```{r F beta div setup 1470, include = FALSE, warning = FALSE}
# subset to first and last days
WCF.1470 <- subset_samples(WCF.rare, day == "14" | day == "70")

# bray curtis
BrayDistF.1470 <- distance(WCF.1470, method = "bray") # create bray dist matrix
BrayOrdF.1470 <- ordinate(WCF.1470, "NMDS", distance = BrayDistF.1470) # make NMDS ordination
BrayPlotF.1470 <- plot_ordination(WCF.1470, BrayOrdF.1470, justDF = TRUE) # make base NMDS dataframe for plotting
```

Plots colored by group:  
```{r F by group 1470, echo = FALSE}
# bray curtis
BrayPlotF.1470 %>%
  ggplot(aes(x=NMDS1, y=NMDS2, color = group)) +
  geom_point(size=4, aes(shape = day)) + sc +
 # geom_line(aes(group = bird.code)) +
#  stat_ellipse(aes(group = day)) +
  stat_ellipse(aes(group = group)) +
  labs(shape = "day") + 
  ggtitle("Bray Curtis, days 14 and 70")
```

```{r F by group 1470 additive, include = FALSE}
# additive plots for talks

# PR only
BrayPlotF.1470 %>%
  ggplot(aes(x=NMDS1, y=NMDS2, color = group, alpha = group)) +
  geom_point(size=4, aes(shape = day)) + sc +
 # geom_line(aes(group = bird.code)) +
#  stat_ellipse(aes(group = day)) +
  stat_ellipse(aes(group = group)) +
...
```{r F adonis, echo = FALSE, warning = FALSE}
# remove DI from the phyloseq
WCF.noDI <- subset_samples(WCF.rare, group != "DI")

# create new distance matrix
BrayDistF.noDI <- distance(WCF.noDI, method = "bray") # create bray dist matrix

# run adonis
adonis2(BrayDistF.noDI ~ group * day + sex + year + bird.code, data = meta(WCF.noDI))
```
...
```{r F pairwise Adonis, echo = FALSE}
# pairwise by group
pairwise.adonis(BrayDistF.noDI, meta(WCF.noDI)$group)

# pairwise by day
pairwise.adonis(BrayDistF.noDI, meta(WCF.noDI)$day)

# pairwise by year
pairwise.adonis(BrayDistF.noDI, meta(WCF.noDI)$year)
```

The pairwise by day results give some more support to the idea of convergence.
...
# bray curtis
BrayPlotT %>%
  ggplot(aes(x=NMDS1, y=NMDS2, color = group)) + sc +
  geom_point(size=4, aes(shape = day)) +
  labs(shape = "day") +
  stat_ellipse(aes(group = group)) +
  ggtitle("Bray Curtis")
...
```{r additive T by group and day, include = FALSE}
# additive plots for talks

# bray curtis PR only
BrayPlotT %>%
  ggplot(aes(x=NMDS1, y=NMDS2, color = group, alpha = group)) + sc +
  geom_point(size=4, aes(shape = day)) +
  labs(shape = "day") +
  stat_ellipse(aes(group = group)) +
  scale_alpha_manual(values = c(0, 0, 1)) +
...
Next I want to see if PERMANOVA identifies whether the composition is significant using the same model structure found for alpha diversity (group * day + sex + year), and including bird.code as a fixed effect because adonis2 cannot do random effects. I plan to move this to Primer7 eventually which should be able to handle random effects.

```{r T adonis, echo = FALSE, warning = FALSE}
# run adonis
adonis2(BrayDistT ~ group * day + sex + year + bird.code, data = meta(WCT.rare))
```

Next step will be post-hoc tests between each of the groups. For this I am using the pairwiseAdonis package.

```{r T pairwise Adonis, echo = FALSE}
# pairwise by group
pairwise.adonis(BrayDistT, metaT.rare$group)
```
